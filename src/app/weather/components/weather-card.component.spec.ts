...
describe('WeatherCardComponent', () => {
  ...
  const mockWeatherData = {
    ...
    data: {
      ...
      daily: {
        time: Array(14)
          .fill(null)
          .map((_, i) => `2024-01-${String(i + 1).padStart(2, '0')}`),
  ...
  beforeEach(async () => {
    weatherServiceMock = {
      ...
      getCityPrecipitationHistory: vi.fn().mockReturnValue(
        of({
          daily: {
      ...
      getCityTemperatureHistory: vi.fn().mockReturnValue(
        of({
          daily: {
            time: ['2024-01-01', '2024-01-02', '2024-01-03'],
    ...
    await TestBed.configureTestingModule({
      imports: [CommonModule, HistoryPrecipitationModalComponent, HistoryTemperatureModalComponent],
      providers: [
  ...
  it('should initialize with correct default values', () => {
    expect(component.showHistoryPrecipitationModal).toBe(false);
    expect(component.showHistoryTemperatureModal).toBe(false);
    expect(component.isLoadingHistoryPrecipitaion).toBe(false);
    expect(component.isLoadingHistoryTemperature).toBe(false);
    expect(component.selectedCityForModal).toBe('');
    expect(component.historyPrecipitation).toEqual([]);
    expect(component.historyTemperature).toEqual([]);
  });
  ...
  describe('showHistoryPrecipitation', () => {
    it('should not show modal when city not found', () => {
      component.showHistoryPrecipitation('UnknownCity');
      expect(component.selectedCityForModal).toBe('');
      expect(component.showHistoryPrecipitationModal).toBe(false);
    });
    ...
    it('should load precipitation history and update state', () => {
      ...
      weatherServiceMock.getCityPrecipitationHistory.mockReturnValueOnce(
        of({
          daily: {
            time: ['2024-01-01', '2024-01-02', '2024-01-03'],
            precipitation_sum: [10, 5, 15],
      ...
      vi.spyOn(WeatherUtils, 'calculateCumulativeHistoryPrecipitation').mockReturnValueOnce(
        mockHistory,
      );

      component.showHistoryPrecipitation('Rennes');

      expect(component.historyPrecipitation).toEqual(mockHistory);
      expect(component.isLoadingHistoryPrecipitaion).toBe(false);
    });
  ...
  describe('showHistoryTemperature', () => {
    it('should not show modal when city not found', () => {
      component.showHistoryTemperature('UnknownCity');
      expect(component.selectedCityForModal).toBe('');
      expect(component.showHistoryTemperatureModal).toBe(false);
    });
    ...
    it('should load temperature history and update state', () => {
      ...
      weatherServiceMock.getCityTemperatureHistory.mockReturnValueOnce(
        of({
          daily: {
      ...
      component.showHistoryTemperature('Rennes');

      expect(component.historyTemperature).toEqual(mockHistory);
      expect(component.isLoadingHistoryTemperature).toBe(false);
    });
  ...
  describe('closeHistoryPrecipitationModal', () => {
    it('should reset precipitation history modal state', () => {
      component.showHistoryPrecipitationModal = true;
      component.historyPrecipitation = [{ date: '2024-01-01', precipitation: 10, cumulative: 10 }];
      component.selectedCityForModal = 'Rennes';
      component.isLoadingHistoryPrecipitaion = true;

      component.closeHistoryPrecipitationModal();
  ...
  describe('closeHistoryTemperatureModal', () => {
    it('should reset temperature history modal state', () => {
      component.showHistoryTemperatureModal = true;
      component.historyTemperature = [
        { date: '2024-01-01', temperature_min: 10, temperature_max: 15 },
      ];
      component.selectedCityForModal = 'Rennes';
      component.isLoadingHistoryTemperature = true;

      component.closeHistoryTemperatureModal();
