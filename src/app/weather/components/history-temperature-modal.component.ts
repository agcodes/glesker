...
@Component({
  selector: 'app-history-temperature-modal',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './history-temperature-modal.component.html',
  styleUrl: './history-temperature-modal.component.css',
})
export class HistoryTemperatureModalComponent implements AfterViewInit, OnChanges {
  @Input() cityName: string = '';
  @Input() data: { date: string; temperature_max: number; temperature_min: number }[] = [];
    ...
    this.chart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'temp. max',
            data: this.data.map((d) => d.temperature_max),
            borderColor: '#e81a1a',
          ...
          {
            label: 'temp. min',
            data: this.data.map((d) => d.temperature_min),
            borderColor: '#1a73e8',
